...
const DestinationCard = ({ image, count, title }) => {
    return (
        <Link href={`/destinations/${title}`} className='text-decoration-none'>
            <div className="destination-card d-flex align-items-center bg-white shadow-sm overflow-hidden h-100">
                ...
                <div className="card-details p-4 flex-grow-1">
                    <div className="badge-hotel d-inline-flex align-items-center mb-2 px-2 py-1 rounded-pill">
                        ...
                        <span className="count-text">{count} Available Hotels</span>
                    </div>
    ...
    return (
        <div className="trip-card bg-white h-100 shadow-sm border-0 position-relative d-flex flex-column">
            ...
            <div className="card-media position-relative overflow-hidden">
                {isFeatured && <span className="featured-badge featured-badge-orange position-absolute fw-semibold">Featured</span>}
                <button className="bookmark-btn position-absolute d-flex align-items-center justify-content-center border-0 rounded-circle text-white" aria-label="Bookmark destination">
                    <Bookmark size={24} />
                </button>

                <div className="img-wrapper">
                    {image?.src ? (
                        <Image src={image} alt={title} className="card-main-img img-fluid" />
                    ) : (
                ...
                {hostAvatar && (
                    <div className="host-avatar-wrapper position-absolute">
                        {hostAvatar.src ? (
                            <Image src={hostAvatar} alt="Host" width={32} height={32} className="rounded-circle border border-white" />
                        ) : (
                            <img src={hostAvatar} alt="Host" className="rounded-circle border border-white" />
                        )}
                    </div>
                )}
            </div>
            ...
            <div className="card-body p-4 d-flex flex-column flex-grow-1">
                <div className="rating-row d-flex align-items-center mb-2">
                    <div className="stars-group text-warning d-flex gap-1 me-2">
                        {[...Array(5)].map((_, i) => (
                            <Star
                                key={i}
                                size={14}
                                className={i < Math.floor(rating) ? "text-warning" : "text-muted-light"}
                    ...
                    <span className="reviews-count ms-1 text-muted">({reviewsCount} reviews)</span>
                </div>
                ...
                <div className="card-footer-row mt-auto pt-3 border-top d-flex align-items-center justify-content-between">
                    <div className="price-box">
                        {originalPrice && <span className="text-decoration-line-through text-muted small me-2">₹{originalPrice}</span>}
            ...
            <div className="card-footer-profile d-flex align-items-center justify-content-between pt-2">
                <div className="d-flex align-items-center gap-3">
                    <div className="avatar-frame flex-shrink-0 position-relative overflow-hidden rounded-circle">
                        {avatar?.src ? (
                            <Image src={avatar} alt={name} className="img-fluid object-cover w-100 h-100" />
                        ) : (
                            <img src={avatar} alt={name} className="img-fluid object-cover w-100 h-100" />
                        )}
                    </div>
                    <div className="profile-meta">
                        <h5 className="profile-name fw-bold mb-0 text-dark">{name}</h5>
                        <span className="profile-role text-uppercase text-muted small tracking-wider">{role}</span>
                    </div>
                </div>
                ...
                <div className="quote-mark-icon select-none">
                    <svg width="48" height="38" viewBox="0 0 48 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 21.6562C0 9.71875 9.09375 0.5 20.3438 0.5V7.6875C13.5 7.6875 9.1875 12.8438 9.1875 19.4062H20.3438V37.5H0V21.6562ZM27.6562 21.6562C27.6562 9.71875 36.75 0.5 48 0.5V7.6875C41.1562 7.6875 36.8438 12.8438 36.8438 19.4062H48V37.5H27.6562V21.6562Z" fill="#E2E8F0" />
                    </svg>
                </div>
            </div>
        </div>
...
const CommonBanner = ({
    ...
    const bannerStyle = {};
    if (bgImage) {
        const imgUrl = typeof bgImage === 'object' && bgImage.src ? bgImage.src : bgImage;
    ...
    return (
        ...
        >
            ...
            <Container className="position-relative z-2 text-white">
                <Row className={`justify-content-${textAlign === 'center' ? 'center' : textAlign === 'right' ? 'end' : 'start'}`}>
                    <Col md={10} className={`text-${textAlign}`}>
                        ...
                        {breadcrumbs.length > 0 && (
                            <nav aria-label="breadcrumb" className="d-inline-flex align-items-center">
                                <ol className="banner-breadcrumbs-list d-flex align-items-center m-0 p-0 list-unstyled fw-semibold">
                                    {breadcrumbs.map((crumb, index) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {!isLast && crumb.url ? (
                                                    <Link href={crumb.url} passHref className='text-decoration-none crumb-link text-white'>
                                                        {crumb.label}
                                                    </Link>
                                                ) : (
                                                    <span>{crumb.label}</span>
                                                )}
                                                {!isLast && <span className="crumb-separator mx-2">—</span>}
                                            </li>
                                        );
                                    })}
                                </ol>
